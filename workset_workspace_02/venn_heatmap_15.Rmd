...
i used GO term and 'dftest' to create a subset - however, this does not contain CXCL13 and some other immune-related genes have gone missing after the subsetting - this may be especially underpowered as this includes D3 -- may be interesting to do this with just the two successful donors, but for now, this is not an appropriate form of analysis 

Thinking just going back to basics

There was some IgG signalling and other signalling pathways were indicated as enriched in the dataset. 

relatively less convincing as this also shows other genes that are upreg/downreg that are not related to the study

maybe another interesting method would be the latent factorisation but may not be appropriate at this time.
...
```{r}
# retrieved <- AnnotationDbi::select(org.Hs.eg.db, keytype="GOALL", keys=c("GO:0045321","GO:0001816"), columns="SYMBOL")
# head(retrieved)
```
